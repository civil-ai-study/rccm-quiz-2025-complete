...
def manual_urban_department_test():
    """都市計画部門の厳格なテスト"""
    print("🚀 都市計画部門(4-2) 厳格な手作業テスト開始")
    print("=" * 80)
    ...
    print("\nStep 2: 都市計画部門問題データ読み込み・検証")
    all_urban_questions = []
    category_target = "都市計画及び地方計画"
    ...
    for file_path in year_files:
        ...
        try:
                    ...
                    with open(file_path, 'r', encoding=encoding) as f:
                        ...
                        for row in reader:
                            if row.get('category', '').strip() == category_target:
                                ...
                                row['year'] = year
                                year_urban_questions.append(row)
                        
                        file_questions = year_urban_questions
            ...
            all_urban_questions.extend(file_questions)
            test_log["steps"].append({
                "step": f"2-{year}",
                "status": "PASSED",
    ...
    print(f"\n✅ 都市計画部門問題データ読み込み完了: {len(all_urban_questions)}問")
    
    if len(all_urban_questions) < 30:
        print(f"❌ CRITICAL: 都市計画部門問題数不足 (必要30問、取得{len(all_urban_questions)}問)")
        test_log["steps"].append({
            "step": 2,
            "status": "FAILED",
            "reason": f"問題数不足: {len(all_urban_questions)}問"
        })
        return False
    ...
    # カテゴリ完全一致チェック
    category_check = [q['category'] for q in all_urban_questions]
    ...
    test_log["steps"].append({
        "step": 3,
        "status": "PASSED",
        "total_questions": len(all_urban_questions),
    ...
    result_10 = execute_specialist_session(all_urban_questions, 10, category_target, test_log)
    
    print("\nStep 5: 20問セッション実行テスト")  
    time.sleep(1)
    result_20 = execute_specialist_session(all_urban_questions, 20, category_target, test_log)
    
    print("\nStep 6: 30問セッション実行テスト")
    time.sleep(1)
    result_30 = execute_specialist_session(all_urban_questions, 30, category_target, test_log)
    ...
    # 結果保存
    report_file = f"manual_urban_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(report_file, 'w', encoding='utf-8') as f:
        json.dump(test_log, f, ensure_ascii=False, indent=2)
    
    print(f"\n📊 都市計画部門テスト結果:")
    print(f"  - 総問題数: {len(all_urban_questions)}問")
...
if __name__ == "__main__":
    success = manual_urban_department_test()
