...
def manual_tunnel_department_test():
    """トンネル部門の厳格なテスト - 10/20/30問完走最優先"""
    print("🚀 トンネル部門(4-2) 厳格な手作業テスト開始")
    print("=" * 80)
    ...
    print("\nStep 2: トンネル部門問題データ読み込み・検証")
    print("⏱️ ウルトラシンクで各年度データを丁寧に処理中...")
    all_tunnel_questions = []
    category_target = "トンネル"
    ...
    for file_path in year_files:
        ...
        try:
                    ...
                    with open(file_path, 'r', encoding=encoding) as f:
                        ...
                        for row in reader:
                            if row.get('category', '').strip() == category_target:
                                ...
                                row['year'] = year
                                year_tunnel_questions.append(row)
                        
                        file_questions = year_tunnel_questions
            ...
            all_tunnel_questions.extend(file_questions)
            test_log["steps"].append({
                "step": f"2-{year}",
                "status": "PASSED",
    ...
    print(f"\n✅ トンネル部門問題データ読み込み完了: {len(all_tunnel_questions)}問")
    
    if len(all_tunnel_questions) < 30:
        print(f"❌ CRITICAL: トンネル部門問題数不足 (必要30問、取得{len(all_tunnel_questions)}問)")
        test_log["steps"].append({
            "step": 2,
            "status": "FAILED",
            "reason": f"問題数不足: {len(all_tunnel_questions)}問"
        })
        return False
    ...
    # カテゴリ完全一致チェック
    category_check = [q['category'] for q in all_tunnel_questions]
    ...
    test_log["steps"].append({
        "step": 3,
        "status": "PASSED",
        "total_questions": len(all_tunnel_questions),
    ...
    time.sleep(2)
    result_10 = execute_complete_session_test(all_tunnel_questions, 10, category_target, test_log)
    
    print("\nStep 5: 20問セッション実行テスト")
    print("⏱️ ウルトラシンクで20問完走テスト実行中...")  
    time.sleep(2)
    result_20 = execute_complete_session_test(all_tunnel_questions, 20, category_target, test_log)
    
    print("\nStep 6: 30問セッション実行テスト")
    print("⏱️ ウルトラシンクで30問完走テスト実行中...")
    time.sleep(2)
    result_30 = execute_complete_session_test(all_tunnel_questions, 30, category_target, test_log)
    ...
    # 結果保存
    report_file = f"manual_tunnel_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(report_file, 'w', encoding='utf-8') as f:
        json.dump(test_log, f, ensure_ascii=False, indent=2)
    
    print(f"\n" + "="*80)
    print(f"📊 トンネル部門テスト結果:")
    print(f"  - 総問題数: {len(all_tunnel_questions)}問")
...
if __name__ == "__main__":
    success = manual_tunnel_department_test()
