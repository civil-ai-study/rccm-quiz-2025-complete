...
# -*- coding: utf-8 -*-
"""
ウルトラシンク Task 8-9: CSRFトークン問題修正（ASCII安全版）
...
def diagnose_and_fix_csrf_token():
    """CSRF token実装の診断および修正"""
    print("=== Task 8-9: CSRF Token Implementation Fix ===")
    ...
    try:
        ...
        if secret_key_configured and csrf_extensions:
            print("  OK: Basic CSRF setup detected")
        else:
            print("  NG: CSRF setup incomplete")
            return False
        
        print()
        
        # 2. テンプレートのCSRFトークン実装確認
        print("2. Template CSRF Token Implementation Check")
        ...
        try:
            ...
            csrf_patterns = {
                ...
                'name="csrf_token"': 'name="csrf_token"' in template_content,
                'method="POST"': 'method="POST"' in template_content,
                'action="/exam"': 'action="/exam"' in template_content
            }
            
            for pattern, found in csrf_patterns.items():
                status = "OK" if found else "NG"
                print(f"  {status} {pattern}: {found}")
            
            template_check_passed = sum(csrf_patterns.values()) >= 3
            
        except Exception as e:
            print(f"  ERROR: Template analysis failed - {str(e)[:60]}...")
            return False
        
        print()
        
        # 3. 実際のCSRFトークン生成テスト
        print("3. CSRF Token Generation Test")
        ...
        # 4. CSRF検証テスト
        print("4. CSRF Token Validation Test")
            ...
            if token_match:
                ...
                if post_response.status_code == 200:
                    ...
                    if 'CSRF' in error_content.upper():
                        print("    Cause: CSRF validation error")
                    elif 'TOKEN' in error_content.upper():
                        print("    Cause: Token related error")
                    else:
                        print(f"    Cause: Unknown - {error_content[:50]}...")
        ...
        if token_issue or not csrf_validation_passed:
            print("5. CSRF Token Implementation Fix")
            ...
            try:
                ...
                # 次の関数定義まで見つける
                fallback_code_end = app_content.find('\ndef ', fallback_code_start + 1)
                if fallback_code_end == -1:
                    fallback_code_end = app_content.find('\n@', fallback_code_start + 1)
                if fallback_code_end == -1:
                    # ファイル末尾まで
...
'''
            ...
            print("5. CSRF Implementation Status")
...
if __name__ == "__main__":
    ...
    # CSRF診断・修正実行
    fix_success = diagnose_and_fix_csrf_token()
