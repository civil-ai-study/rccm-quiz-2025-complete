...
def complete_10_question_test(department_id, department_name, base_url="http://127.0.0.1:5005"):
    ...
    try:
        ...
        if dept_response.status_code != 200:
            return f"❌ 部門ページアクセス失敗: {dept_response.status_code}"
        
        if "学習開始" not in dept_response.text:
            return f"❌ 部門ページ表示異常: 学習開始ボタンなし"
        ...
        if start_response.status_code != 200:
            return f"❌ 問題開始失敗: {start_response.status_code}"
        
        print("✅ specialist問題開始成功")
        ...
        for i in range(1, 11):
            ...
            if exam_response.status_code != 200:
                return f"❌ 問題{i}取得失敗: {exam_response.status_code}"
            ...
            if progress_element:
                ...
                if f"{i}/10" not in progress_text:
                    return f"❌ 問題{i}の番号表示異常: {progress_text}"
            else:
                return f"❌ 問題{i}の進捗表示なし"
            ...
            if category_match:
                ...
                if category != department_name:
                    return f"❌ 問題{i}で分野混在発見: 期待値'{department_name}' 実際'{category}'"
                
                print(f"  ✅ カテゴリ正常: {category}")
            else:
                return f"❌ 問題{i}でカテゴリ情報なし"
            ...
            if question_element:
                ...
                return f"❌ 問題{i}で問題文なし"
            
            # qid取得
            qid_input = soup.find('input', {'name': 'qid'})
            if not qid_input:
                return f"❌ 問題{i}でqid取得失敗"
            ...
            if answer_response.status_code != 200:
                return f"❌ 問題{i}回答送信失敗: {answer_response.status_code}"
        ...
        if result_response.status_code != 200:
            return f"❌ 結果画面アクセス失敗: {result_response.status_code}"
        
        if "テスト完了" not in result_response.text:
            return f"❌ 結果画面表示異常: テスト完了表示なし"
        
        # 結果詳細確認
        result_soup = BeautifulSoup(result_response.text, 'html.parser')
        dept_info = result_soup.find(string=re.compile(r'部門:'))
        answer_info = result_soup.find(string=re.compile(r'回答数:'))
        
        print("✅ 結果画面到達成功")
        if dept_info:
            print(f"  部門情報: {dept_info.strip()}")
        if answer_info:
            print(f"  回答情報: {answer_info.strip()}")
...
if __name__ == "__main__":
    ...
    if isinstance(result, dict) and result['status'] == 'SUCCESS':
        ...
        print(f"✅ 問題完了: {result['questions_completed']}/10")
        print(f"✅ 分野混在: {'なし' if not result['field_mixing_detected'] else 'あり'}")
