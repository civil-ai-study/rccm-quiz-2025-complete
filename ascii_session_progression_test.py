...
# -*- coding: utf-8 -*-
"""
ASCII Safe Session Progression Test
Purpose: Test if road department progresses 1->2->3 correctly using Flask test client
"""
import sys
import os
...
def ascii_session_progression_test():
    """ASCII Safe Session Progression Test"""
    print("=== ASCII Safe Session Progression Test ===")
    
    with app.test_client() as client:
        # Step 1: Start road department session
        ...
        # Step 3: Navigate to 2nd question
        print("\n3. Question 2 Navigation Test")
        
        # Find next question link
        next_link_match = re.search(r'href="([^"]*exam[^"]*)"', post_html)
        if not next_link_match:
            print("ERROR: Next question link extraction failed")
            return False
        
        next_url = next_link_match.group(1)
        print(f"SUCCESS: Next question URL: {next_url}")
        ...
        if not next_link_match2:
            print("ERROR: Question 3 link extraction failed")
            return False
        ...
        if '3/10' in html3:
            ...
            if 'road' in html3.lower() or 'doro' in html3.lower():
                print("SUCCESS: Field verification - Road department questions correctly displayed")
            else:
                print("WARNING: Potential field mixing - Non-road questions may be displayed")
...
if __name__ == "__main__":
    success = ascii_session_progression_test()
    if success:
