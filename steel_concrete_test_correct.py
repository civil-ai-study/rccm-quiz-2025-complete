...
# -*- coding: utf-8 -*-
"""
鋼構造・コンクリート部門完全テスト（ウルトラシンク・絶対に嘘をつかない）
...
def steel_concrete_complete_test():
    """
    鋼構造・コンクリート部門の完全テスト実行（絶対に嘘をつかない）
    """
    
    print("=" * 100)
    print("鋼構造・コンクリート部門完全テスト（ウルトラシンク・絶対に嘘をつかない）")
    print("=" * 100)
    
    test_results = {
        'timestamp': datetime.now().isoformat(),
        'department': '鋼構造・コンクリート',
        'test_status': 'running',
        'detailed_results': {},
        'errors': [],
    ...
    try:
        ...
        print("\nステップ2: 鋼構造・コンクリート部門セッション開始")
        
        # 部門名をURLエンコード
        department_encoded = urllib.parse.quote('鋼構造・コンクリート')
        ...
        # カテゴリ情報の確認
        steel_concrete_keywords = ["鋼構造", "コンクリート", "鋼", "構造", "RC", "SRC", "PC"]
        for keyword in steel_concrete_keywords:
            if keyword in exam_content:
                categories_found.add("鋼構造・コンクリート")
                print(f"OK 鋼構造・コンクリートカテゴリ確認（キーワード: {keyword}）")
                break
        
        # 他分野の混在チェック
        other_categories = ["道路", "河川", "砂防", "都市計画", "造園", "建設環境", "土質", "基礎", "施工", "上下水道", "森林", "農業", "トンネル"]
        ...
        # カテゴリ一貫性の確認
        if len(categories_found) == 1 and "鋼構造・コンクリート" in categories_found:
            test_results['category_consistency'] = True
            print("OK カテゴリ一貫性確認: 鋼構造・コンクリート部門のみ")
    ...
    result_file = f"steel_concrete_test_results_{timestamp}.json"
    ...
    # 結果サマリー表示
    print("\n" + "=" * 100)
    print("鋼構造・コンクリート部門完全テスト結果サマリー（絶対に嘘なし）")
    print("=" * 100)
...
if __name__ == "__main__":
    result = steel_concrete_complete_test()
