...
# ===== ULTRATHIN区 PHASE 2-2-5-B: 新システムBlueprint統合 =====
# 基礎科目専用新システム Blueprint 登録（既存システム影響ゼロ保証）
try:
    from basic_exam_blueprint import basic_exam_bp
    app.register_blueprint(basic_exam_bp)
    logger.info("✅ ULTRATHIN区: 基礎科目専用システム (/v2/basic_exam/) 統合完了")
except ImportError as e:
    logger.warning(f"⚠️ ULTRATHIN区: 基礎科目専用システム統合スキップ - {e}")
except Exception as e:
    logger.error(f"❌ ULTRATHIN区: 基礎科目専用システム統合エラー - {e}")
# ===== ULTRATHIN区 Blueprint統合終了 =====
