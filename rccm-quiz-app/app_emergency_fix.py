...
# 緊急修正版：本番ポート5005で軽量版を実行
@app.route('/departments/<department_id>/types')
def question_types_emergency(department_id):
    """緊急修正版：軽量版パターンでdepartment_types画面を提供"""
    if department_id not in DEPARTMENT_MAPPING:
        return f"エラー: 部門ID '{department_id}' の問題が見つかりません", 404
    
    department_name = DEPARTMENT_MAPPING[department_id]
...
if __name__ == '__main__':
    print("EMERGENCY: RCCM emergency fix server starting")
    print("PURPOSE: Fix 9 department errors on emergency port 5007")
    print("SUPPORT: Basic + 12 specialist departments")
