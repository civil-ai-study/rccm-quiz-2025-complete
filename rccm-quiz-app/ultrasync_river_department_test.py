#!/usr/bin/env python3
# 🛡️ ULTRASYNC 河川・砂防部門 Flask手動等価テスト
...
def test_river_department():
    """河川・砂防部門 Flask手動等価テスト"""
    
    print('🛡️ ULTRASYNC 河川・砂防部門 Flask手動等価テスト開始')
    print('=' * 70)
    ...
    try:
        ...
        with app.test_client() as client:
            ...
            # 河川・砂防部門固有の問題表示確認
            if '河川' in content or '砂防' in content or '治水' in content or '堤防' in content:
            ...
            # 適切な回答送信エンドポイントを使用
            if 'exam_question' in exam_response.request.path:
...
if __name__ == '__main__':
    success = test_river_department()
    
    if success:
        print('\n🚀 結論: 河川・砂防部門10問完走テストは正常動作します')
        print('🎯 Flask手動等価テストで完全動作確認済み')
