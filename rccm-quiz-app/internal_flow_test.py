#!/usr/bin/env python3
"""
Flaskテストクライアントを使用した内部フローテスト
ユーザー指摘事項の包括的検証（HTTPサーバー不要）
"""
...
class InternalFlowTester:
    def __init__(self):
        self.app = app
        self.app.config['TESTING'] = True
        self.app.config['WTF_CSRF_ENABLED'] = False  # CSRFを無効化してテスト簡略化
    ...
    def extract_qid_from_html(self, html_content):
        """HTMLから問題IDを抽出"""
        match = re.search(r'name="qid" value="([^"]+)"', html_content)
    ...
    def test_1_initial_screen_to_basic_questions(self):
        ...
        try:
            with self.app.app_context():
                ...
                # 2. 道路部門の基礎問題選択
                response = self.client.get('/exam?department=road&type=basic')
                if response.status_code != 200:
                ...
                for question_num in range(1, 11):
                    ...
                    # 問題内容確認
                    if "4-1 基礎科目" not in html_content:
                        self.log_error(f"問題{question_num}内容", "基礎科目の問題が表示されていない")
                        continue
                        
                    # 問題IDを抽出
                    qid = self.extract_qid_from_html(html_content)
                    if not qid:
                    ...
                    if question_num == 2 or question_num == 5:
                        bookmark_response = self.client.post('/bookmark', data={'qid': qid})
                        if bookmark_response.status_code in [200, 302]:
                            self.log_success(f"問題{question_num}復習追加", "復習リストに追加成功")
                            review_count += 1
                            self.review_questions.append(qid)
                        else:
                            self.log_error(f"問題{question_num}復習追加", f"復習リスト追加失敗: {bookmark_response.status_code}")
                    ...
                    answer_response = self.client.post('/exam', data=answer_data)
                    if answer_response.status_code not in [200, 302]:
                        self.log_error(f"問題{question_num}回答", f"回答送信失敗: {answer_response.status_code}")
                        continue
                        
                    # フィードバック画面確認
                    feedback_html = answer_response.get_data(as_text=True)
                    if "正解" in feedback_html or "不正解" in feedback_html:
    ...
    def test_2_results_and_review_list_check(self):
        ...
        try:
            ...
            # 結果画面内容確認
            if "正答数" in html_content and "正答率" in html_content:
                self.log_success("結果画面表示", "結果分析画面が正しく表示")
            else:
                self.log_error("結果画面内容", "結果分析情報が不完全")
                
            # 基礎科目成績表示確認
            if "4-1 基礎科目" in html_content:
            ...
            # 復習リスト内容確認
            bookmark_count = html_content.count('bookmark-item')
            if bookmark_count >= 2:
    ...
    def test_3_review_functionality(self):
        ...
        try:
            ...
            # 復習問題が表示されているか確認
            if "RCCM試験" in html_content and ("選択肢A" in html_content or "option_a" in html_content):
            ...
            if qid:
                ...
                answer_response = self.client.post('/review', data=answer_data)
                if answer_response.status_code in [200, 302]:
                    self.log_success("復習回答", "復習問題の回答処理完了")
                else:
                    self.log_error("復習回答", f"復習問題の回答処理失敗: {answer_response.status_code}")
            else:
                self.log_error("復習回答", "復習問題のIDが取得できない")
    ...
    def test_4_specialist_questions(self):
        ...
        try:
            ...
            # 道路部門の専門科目選択
            response = self.client.get('/exam?department=road&type=specialist')
            if response.status_code != 200:
            ...
            for question_num in range(1, 11):
                ...
                if question_num in [1, 4, 8]:
                    ...
                    if bookmark_response.status_code in [200, 302]:
                        ...
                        self.log_error(f"専門問題{question_num}復習追加", f"復習リスト追加失敗: {bookmark_response.status_code}")
            ...
            for attempt in range(3):  # 3回復習問題にアクセス
                ...
                if qid:
                    ...
                    if answer_response.status_code in [200, 302]:
                        ...
                        self.log_error(f"復習回答{attempt+1}", f"復習問題回答処理失敗: {answer_response.status_code}")
    ...
    def run_comprehensive_flow_test(self):
        """包括的フローテスト実行"""
        print("🔬 RCCM試験問題集アプリ - ユーザー指摘事項の厳重フローテスト（内部実行）")
        print("=" * 80)
