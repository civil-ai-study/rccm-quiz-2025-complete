...
def ultra_safe_second_replacement_prep():
    ...
    safety_checks = {
        ...
        'correct_keys': all('exam_question_ids' in next_target['lines'][0],
                           'exam_current' in next_target['lines'][1],
                           'exam_category' in next_target['lines'][2]),
    ...
    # インデントを保持
    indent = len(lines[next_target['start_line'] - 1]) - len(lines[next_target['start_line'] - 1].lstrip())
