...
左側問題番号チェック(1/10-10/10)・結果確認・分野混在チェック
"""
    ...
    try:
        ...
        for i in range(10):
            ...
            if f"{current_question_num}/10" in exam_content:
                ...
                print(f"✅ 問題番号確認: {current_question_num}/10")
            else:
                test_results['errors'].append(f"問題番号{current_question_num}/10が見つからない")
                print(f"❌ 問題番号{current_question_num}/10が見つからない")
            ...
            if i < 9:  # 最後の問題以外
                ...
                if answer_response.status_code == 200:
                    print(f"✅ 問題{current_question_num}回答処理成功")
                    ...
                    for other_cat in other_categories:
                        if other_cat in answer_content:
                            ...
                            print(f"⚠️ 分野混在検出: {other_cat}")
                else:
                    test_results['errors'].append(f"問題{current_question_num}回答処理失敗: {answer_response.status_code}")
                    print(f"❌ 問題{current_question_num}回答処理失敗: {answer_response.status_code}")
